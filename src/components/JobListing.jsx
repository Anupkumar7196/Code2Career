  ...
  return (
    ...
    <section className='w-full lg:w-3/4 text-gray-800 max-lg:px-4'>
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4'>
                {jobs.slice((currentPage-1)*4, currentPage * 4).map((job, index)=>(
    ...
    {/* Pagination */}
        {jobs.length > 0 && (
            <div className='flex items-center justify-center space-x-2 mt-10'>
                ...
                {Array.from({length:Math.ceil(jobs.length/4)}).map((_, index)=>(
                    <a href="#job-list">
                ...
                <a href="#job-list">
                    <img onClick={()=> setCurrentPage(Math.min(currentPage+1, Math.ceil(jobs.length / 4)))} src={assets.right_arrow_icon} alt="" />
                </a>
            </div>)}
    </section>
    </div>
